...
we can consider alcohol consumption , beer tax and fatalities from table
1

we consider baptist and mormon from table 2 as second table has
standarized values.

## Cluster Analysis (by average)

**Cluster 1**: This cluster is characterized by an average consumption
of 1.47 alcoholic drinks per person, with 323 alcohol-related car
crashes on average. The average beer tax is relatively high at 0.930. It
predominantly comprises Baptists rather than Mormons.

**Cluster 2**: Cluster 2 exhibits a slightly higher average consumption
of alcoholic drinks per person (1.72) compared to Cluster 1, with a
lower average of 117 alcohol-related car crashes. The average beer tax
is notably lower at 0.237. This cluster consists of somewhat more
Mormons than Baptists.

**Cluster 3**: In Cluster 3, the average consumption of alcoholic drinks
per person is 1.39, with 113 alcohol-related car crashes on average. The
average beer tax falls in between the previous clusters at 0.501. This
cluster has similar proportions of Baptists and Mormons.

## Cluster Analysis (by average)

**Cluster 4**: This cluster demonstrates the highest average consumption
of alcoholic drinks per person (2.18) among the clusters, with 142
alcohol-related car crashes on average. The average beer tax is similar
to Cluster 2 at 0.245 . Similar to Cluster 3, it contains similar
amounts of Baptists and Mormons.

**Cluster 5**: Cluster 5 has an average consumption of 1.6 alcoholic
drinks per person, with 473 alcohol-related car crashes on average. The
average beer tax is 0.270. This cluster contains more Mormons than
Baptists.

**Cluster 6**: Finally, Cluster 6 displays the lowest average
consumption of alcoholic drinks per person (1.24), with 295
alcohol-related car crashes on average. The average beer tax is 0.350.
Significantly, this cluster contains more Baptists than Mormons.

## Cluster analysis (by variable) {.smaller}

::: panel-tabset
#### Car Crash Fatalities

In this chart it is shown that cluster 5 had the highest number of car
crash fatalities related to alcohol followed by cluster 1 while clusters
2 and 3 had the lowest amount.

```{r}
y=clustertable$cluster
x=clustertable$afatal
...
In this chart, cluster 4 had the highest number of alcohol consumed
while cluster 6 had the lowest amount. The second highest was cluster 2.

```{r}
y=clustertable$cluster
x=clustertable$spirits
data=as.data.frame(x)
data$y=y
...
in this chart, clusters 1 and 6 had the highest population of Baptists
...
## Cluster analysis (state level) {.smaller}
...
## State map based on clusters {.smaller}
...
From the above map we can see that the states are divided based on
clusters.

we can observe that southern states belongs to cluster 1, where as
western states are outliers hence there are no colour for the state, so
we can say that those states doesnot belong to any clusters. cluster 2
also belongs to western part of usa.

most of the central part states belong to cluster 3.

cluster 6 belongs to central-east part of usa.

Cluster 4 is primarily in the Northeast of the U.S. Cluster 5 is in the
Great Lakes and MidEast area.

## CONCLUSION

-   The K-means clustering algorithm is known for its simplicity and is
    applied in clustering datasets from different domains.
...
-   K-means clustering was utilized as an effective method for grouping
    data to extract insights and gives more weight to the bigger
    clusters
...
-   Six clusters were generated to interpret the trends related to
    alcohol-related car crashes, considering factors such as alcohol
    consumption, religion, and beer tax.

-   The clusters helped to create sub-regions within the United States,
...
-   The decision-making process for determining the optimal number of
    ...
    Statistic.
-   K-means clustering is essential in the analysis of the "Fatalities"
    dataset as it facilitates the extraction of meaningful insights,
    aids in decision-making processes, and contributes to a deeper
    understanding of the factors contributing to fatal car crashes and
    their implications.
