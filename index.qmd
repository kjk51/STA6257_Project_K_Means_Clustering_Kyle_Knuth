...
the age ranges and religion variables

Cluster 1:

-   consists of average of 1.46 alcoholic drinks consumed per person

-   There was an average of 339.76 alcohol related car crashes with most
    of them being in the 21-24 age range

-   This cluster consisted of mainly Baptists rather than Mormons

Cluster 2:

-   Consists of an average of 1.64 alcoholic drinks consumed per person

-   There was an average of 510.31 alcohol related car crashes with most
    of them also being in the 21-24 age range

-   This cluster consists of somewhat more Baptists than Mormons

Cluster 3:

-   Consists of an average of 1.72 alcoholic drinks consumed per person

-   There was an average of 117.09 alcohol related car crashes with most
    of them being in the 15-17 age range

-   This cluster consists of mainly Baptists than Mormons

Cluster 4:

-   Consists of an average of 1.09 alcoholic drinks consumed per person

-   There was an average of 176.92 alcohol related car crashes with most
    of them being spread pretty evenly between all the age ranges
    (15-24)

-   This cluster contains mainly Mormons

Cluster 5:

-   Consists of an average of 1.95 alcoholic drinks consumed per person

-   There was an average of 47.37 alcohol related car crashes with most
    of them being between 18-24

-   This cluster contains more Mormons than Baptists

Cluster 6:

-   Consists of an average of 2.15 alcoholic drinks consumed per person

-   There was an average of 200.78 alcohol related car crashes with most
    of them being between all the age groups (15-24)

-   This cluster contains significantly more Mormons than Baptists

### Cluster analysis (by variable)

In this first chart it is shown that cluster 4 had the highest number of
car crash fatalities related to alcohol while cluster 2 had the lowest
amount. Cluster 3 had the second highest amount.
...
In this next chart, cluster 5 had the highest number of alcohol consumed
while clusters 1 had the lowest amount. The second highest was cluster
2.
...
As shown below, cluster 3 had the highest population of Baptists while
cluster 1 had the lowest amount. The rest of the clusters had similar
populations of Baptist.
...
In this chart, cluster 6 had the highest population of Mormons and
cluster 2 had the lowest population.
...
The highest number of fatal car crashes in the age range of 15 to 17
occurred in cluster 4 while the lowest number was in cluster 2.

```{r}
y=clustertable2$cluster
x=clustertable2$fatal1517
data=as.data.frame(x)
data$y=y

ggplot(data, aes(x = y, y = x)) +
  geom_bar(stat = "identity", fill = y) + 
  labs(x = "Cluster", y = "Fatal1517")

```

The highest number of fatal car crashes in the age range of 18 to 20
occurred in cluster 4 while the lowest number was in cluster 2.

```{r}
y=clustertable2$cluster
x=clustertable2$fatal1820
data=as.data.frame(x)
data$y=y

ggplot(data, aes(x = y, y = x)) +
  geom_bar(stat = "identity", fill = y) + 
  labs(x = "Cluster", y = "Fatal1820")

```

Lastly, the highest number of fatal car crashes in the age range of 21
to 24 occurred in cluster 4 while the lowest number was in cluster 2.

```{r}
y=clustertable2$cluster
x=clustertable2$fatal2124
data=as.data.frame(x)
data$y=y

ggplot(data, aes(x = y, y = x)) +
  geom_bar(stat = "identity", fill = y) + 
  labs(x = "Cluster", y = "Fatal2124")
