...
method, Gap statistic, and Canopy algorithm. The Silhouette method is
used to visually find the number of clusters by using the silhouette
...
indicates the optimal number of clusters [@lee2022identifying]. The last
method of determining the optimal number of clusters is the Canopy
algorithm. The Canopy algorithm is the best method for large and complex
data sets and enhances the fault tolerance [@yuan2019research]. When it
comes to smaller and less complex data-sets, the other methods are
better.
...
method. According to this it is 9 clusters.

```{r}
fviz_nbclust(df2, kmeans, method = "wss")
```

Here we are using the Silhouette method which also shows 9 clusters.

```{r}
silhouette2 = pamk(df2)
plot(pam(df, silhouette2$nc))
```
