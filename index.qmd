...
means the data was assigned to the wrong cluster [@song2023novel]. The
Gap statistic finds the optimal number of clusters by comparing the
...
cluster[@song2023novel]. The Silhouette Coefficient is shown below in
equation 1:

<center>$$S(x_i)=b(x_i)-a(x_i)/max\{b(x_i),\:a(x_i)\} \quad (1)$$</center>
...
[@syakur2018integration]\]. The Elbow method is shown below in equation
2:

<center>$$
SSE=\sum^K_{K=1}\sum_{x_i\in S_K}||x_i-C_k||_2^2 \quad (2)
$$</center>

x: data present in the cluster
...
<center>$$d(x,y)=\sqrt{\sum|x_i-y_i|^2} \quad (3)$$</center>
...
Here we check for outliers and scale the data. Although k-means is
...
method. According to this it could 6 clusters.

```{r}
fviz_nbclust(cleaned_df2, kmeans, method = "wss")
```

Here we are using the Silhouette method which shows 6 or 10 clusters.

```{r}
fviz_nbclust(cleaned_df2, kmeans, method = "silhouette")
```

Here is the Gap statistic showing 6 clusters.
...
The silhouette test for this new data frame shows 5 or 9 clusters as
being optimal.
...
Here the Gap statistic is showing 5, 6, and 7 clusters.

```{r}
fviz_nbclust(cleaned_df3, kmeans, method = "gap_stat")
```

We are looking for the best variables to use when considering our
...
In this graph the optimal number of clusters is 5 or 6.

```{r}
fviz_nbclust(cleaned_dfnew, kmeans, method = "silhouette")
```

Here the Gap statistic is showing 5 clusters.
...
```{r}
km3 <- kmeans(cleaned_dfnew, centers= 6, nstart = 25)
