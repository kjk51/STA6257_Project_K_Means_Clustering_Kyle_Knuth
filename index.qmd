...
-   The average of the points in a cluster is the centroid of the
    cluster meaning that outliers will dramitically affect clustering 
...
of baptist, the amount of mormons, the age range of fatal car crashes,
and alcohol related car crashes. These variables were chosen to observe
the relationship between alcohol consumption and religion in regards to
fatal car crashes.
...
As shown in the boxplot below there are some outliers that need to be
removed in our dataset.
...
For the next step in our analysis we are using Recursive Feature
Elimination (RFE) to determine which variables to keep based off a
response variable. RFE will help find the best combination of variables
to use based off a limited version of the Fatalities dataset. The
limited Fatalities dataset only includes numerical variables and
fatality variables related to age and alcohol since the other fatality
variables would be too similar. We are also removing the population
variables. In this case we are interested in the number of vehicle
fatalities that involved alcohol.

For this part of the analysis we are checking for any missing and null
values in the dataset. There appears to be two missing values in the
dataset so we will remove them.

```{r}
print(c(sum(is.na(df3)),sum(is.null(df3))))
```
...
According to recursive feature elimination, the 5 best variables to use
in relation to "afatal" from this dataset are "fatal1517", "fatal1820",
"fatal2124", "mormon", and "baptist".
...
response=df3$afatal
result=rfe(x=predictors, y = response, sizes=18, rfeControl = control, metric = "RMSE", method="cv")
print(result)
```

Based off the RFE analysis we created a new dataframe using those
variables.
...
As done previously, all outliers are removed and the dataframe is
scaled.

```{r}
column_names2=c('baptist','mormon', 'fatal1517','fatal1820', 'fatal2124', 'afatal')
cleaned_df_3=elimination(df3,column_names2)
cleaned_df3=as.data.frame(scale(cleaned_df_3))
boxplot(cleaned_df3)
```

Again, elbow method is used and it appears the optimal number of
clusters for this new dataframe is 5.

```{r}
fviz_nbclust(cleaned_df3, kmeans, method = "wss")
```

The silhouette method for this new data frame shows 5 or 9 clusters as
being optimal.
...
Finally, the Gap statistic is showing 5, 6, and 7 clusters as being
optimal.

```{r}
fviz_nbclust(cleaned_df3, kmeans, method = "gap_stat")
```

Overall it appears that the best number of clusters according to all
three methods is 5 clusters.

# Using RFE on our chosen variables

For the final step before clustering the data, we are perfomring RFE on
our original chosen variables.We are looking for the best variables to
use when considering our variables of interest. According to RFE, the 5
best variables to use in relation to "afatal" from this dataset are
"fatal1517", "fatal1820", "fatal2124", "mormon".

```{r}
dfnew=df
predictors2=dfnew[,-which(names(dfnew)=="afatal")]
response2=dfnew$afatal
result2=rfe(x=predictors, y = response, sizes=7, rfeControl = control, metric = "RMSE", method="cv")
print(result2)
```

We created a new dataframe based off of RFE and created a boxplot as
shown below.
...
We are again removing any outliers and scaling the data from our last
dataframe.

```{r}
column_names3=c('spirits','mormon', 'fatal1517','fatal1820', 'fatal2124', 'afatal')
cleaned_df_new=elimination(dfnew,column_names3)
cleaned_dfnew=as.data.frame(scale(cleaned_df_new))
boxplot(cleaned_dfnew)
```

As shown by the elbow method, the optimal number of clusters is 6.

```{r}
fviz_nbclust(cleaned_dfnew, kmeans, method = "wss")
```

In this graph using the Silhouette method, the optimal number of
clusters is 5.
...
Lastly, the Gap statistic is showing 5 clusters as being optimal.

```{r}
fviz_nbclust(cleaned_dfnew, kmeans, method = "gap_stat")
```

Overall for this last dataframe, the optimal number of clusters is 5 as
shown by all three methods.

### Modeling With K-Means Clustering

Now we are using our selected number of clusters for each respective
dataframe to perform k-means clustering. The plot below shows clustering
for our first dataframe consisting of our variables of interest.

```{r}
km <- kmeans(cleaned_df2, centers= 6, nstart = 25)
fviz_cluster(km, data = cleaned_df2)
```
...
```{r}
km2 <- kmeans(cleaned_df3, centers= 5, nstart = 25)
fviz_cluster(km2, data = cleaned_df3)
```

As for our second dataframe, it has a between ss/ total ss of 80.7%.
...
```{r}
km3 <- kmeans(cleaned_dfnew, centers= 5, nstart = 25)
fviz_cluster(km3, data = cleaned_dfnew)
```

Finally, the between ss/ total ss for our final dataframe is 74.9%
...
cleaned_df_3$cluster=km2$cluster
cleaned_df_new$cluster=km3$cluster
clustertable=cleaned_df_2%>%
  group_by(cluster) %>%
  summarise_all("mean")
clustertable2=cleaned_df_3%>%
  group_by(cluster) %>%
  summarise_all("mean")
clustertable3=cleaned_df_new%>%
  group_by(cluster) %>%
  summarise_all("mean")
print(clustertable)
```
